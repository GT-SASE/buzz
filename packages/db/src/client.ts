...
const conn =
  globalForDb.conn ??
  (url
    ? // Serverless-friendly: one connection per instance. Skip prepared
      // statements (transaction-mode poolers reject them). Fail fast if the
      // host will not accept a socket, instead of waiting on TCP defaults.
      postgres(url, {
        max: 1,
        idle_timeout: 60,
        connect_timeout: 5,
